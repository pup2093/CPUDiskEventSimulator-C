...
int main(){
	...
	while(!isEmptyevent(&eventhead) && (eventhead->event != SIMULATION_FINISHED)){
		...
		if(peekevent(&eventhead) == JOB_ARRIVES){
			...
			handle_job_arrival(cpuhead, eventhead, file);

		}else if(peekevent(&eventhead) == CPU_FINISHED){

			handle_finish_cpu(disk1head, disk2head, eventhead, file);

		}else if(peekevent(&eventhead) == DISK1_FINISHED){
			
			handle_finish_disk1(cpuhead, eventhead, file);

		}else if (peekevent(&eventhead) == DISK2_FINISHED){

			handle_finish_disk2(cpuhead, eventhead, file);
		}
		else if(eventhead->time == FIN_TIME){
			break;
		}
